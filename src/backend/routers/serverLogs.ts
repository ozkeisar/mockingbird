...
import { projectsManager, serverLogsManager } from '../managers';
import { logger } from '../utils/utils';

const serverLogsRouter = Router();

serverLogsRouter.get('/:projectName', async (req: Request, res: Response, next: NextFunction) => {
    try {
...
serverLogsRouter.delete('/:projectName', async (req: Request, res: Response, next: NextFunction) => {
    try {
        const {projectName} = req.params

        logger('restRouter delete /:projectName/:serverName/:parentId/:routeId',{ projectName});
