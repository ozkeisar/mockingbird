...
parentsRouter.get(
  '/:projectName/:serverName/:parentId',
  async (req: Request, res: Response) => {
    try {
      const { projectName, serverName, parentId, routeId } = req.params;
      logger('restRouter get /:projectName/:serverName/:parentId', {
        projectName,
        serverName,
        parentId,
        routeId,
      ...
      parent.graphQlRouteHash = Object.values(
      ...
      parent.routesHash = Object.values(parent.routesHash || {}).reduce(
        (acc, item) => {
          acc[item.id] = {
            ...item,
            responsesHash: null,
