...
import { handleCloseServer, handleRestartServer, handleStartServer } from '../actions';
import { logger } from '../utils/utils';

const actionsRouter = Router();

actionsRouter.post('/server/start', async (req: Request, res: Response, next: NextFunction) => {
    try {
...
actionsRouter.post('/server/close', async (req: Request, res: Response, next: NextFunction) => {
    try {
        logger('close server');
...
actionsRouter.post('/server/restart', async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { projectName } = req.body;
