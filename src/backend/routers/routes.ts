...
routesRouter.get('/:projectName/:serverName/:parentId/:routeId', async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { projectName, serverName, parentId, routeId} = req.params;
        logger('restRouter get /:projectName/:serverName/:parentId/:routeId',{ projectName, serverName, parentId, routeId});
        ...
        console.log(error)
        logger('Error get /:projectName/:serverName/:parentId/:routeId',error?.message)
