...
serverRouter.get('/:projectName/:serverName', async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { projectName, serverName } = req.params;
        logger('serverRouter get /:projectName/:serverName',{ projectName, serverName });
        ...
        server.parentRoutesHash = Object.values(server.parentRoutesHash).reduce((acc, item)=>{
            acc[item.id] = {
                ...item,
