...
function groupRoutes(routes: Endpoint[]): { [key: string]: Endpoint[] } {
  ...
  const result = routes.map((route) => {
    ...
    const segments = [...route.path.split('/').filter(Boolean), ''];
    // Use the first segment as the group key
    const routeGrouping = segments.reduce(
      (acc, segment, i) => {
        const group = groups[i];
        const amountInGroup = group.get(acc.groupKey)?.length || 0;

        let score = amountInGroup ** i + i;
        ...
        if (
          (amountInGroup === 1 && i !== 0) ||
          (segment === 'api' && segments[i + 1]?.startsWith('v'))
        ) {
          score = 0;
        }
    ...
    groups[routeGrouping.selected.groupIndex]
      .get(routeGrouping.selected.group)!
      .push(route);
...
export const buildRoutesFromSwaggerData = async (
  projectName: string,
  serverName: string,
  ...
  await Promise.all(
    Object.keys(grouped).map(async (parentPath) => {
      const parent = await handleCreateParent(
        projectName,
        serverName,
        parentPath,
      );
      ...
      await Promise.all(
        grouped[parentPath].map(async (routeData) => {
          try {
...
export const addRoutesFromSwaggerUrl = async (
  projectName: string,
  serverName: string,
  swaggerUrl: string,
) => {
...
export const addRoutesFromSwaggerJson = async (
  projectName: string,
  serverName: string,
  swaggerJson: any,
) => {
