...
import { readAppSettings } from "../../backend/utils";
...
export const serverEvents = (socket: Socket)=>{
    socket.on(EVENT_KEYS.CLOSE_SERVER, async (event, arg) => {
        try {
            handleCloseServer()
        } catch (error) {
    ...
    socket.on(EVENT_KEYS.START_SERVER, async (arg) => {
        try {
            ...
            socket.emit(EVENT_KEYS.START_SERVER, {success: false, error, projectName: arg?.projectName});
        }
    });
    ...
    socket.on(EVENT_KEYS.RESTART_SERVER, async (arg) => {
        ...
        try {
            ...
            socket.emit(EVENT_KEYS.START_SERVER, {success: true, host, projectName});
