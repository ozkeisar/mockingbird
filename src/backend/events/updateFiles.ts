...
import { createNewServer, deleteParent, deleteServer, readProjectSettings, readServerData, readServerSettings, updateProjectSettings, updateProjectSupportedDataVersion, updateRouteParentFile, updateServerSettings } from "../../backend/utils";
...
export const updateFilesEvents = (socket: Socket)=>{
    socket.on(EVENT_KEYS.UPDATE_ROUTES_FILE, async (arg) => {
        try {
            ...
            socket.emit(EVENT_KEYS.UPDATE_ROUTES_FILE, {success: true, content, filename, projectName, serverName, hasDiffs});
    ...
    socket.on(EVENT_KEYS.UPDATE_SERVER_SETTINGS, async (arg) => {
        try {
            ...
            socket.emit(EVENT_KEYS.UPDATE_SERVER_SETTINGS, {success: true, serverSettings, projectName, serverName, hasDiffs});
    ...
    socket.on(EVENT_KEYS.CREATE_SERVER, async (arg) => {
        try {
            ...
            socket.emit(EVENT_KEYS.CREATE_SERVER, {success: true, projectName, server, hasDiffs });
    ...
    socket.on(EVENT_KEYS.DELETE_SERVER, async (arg) => {
        try {
            ...
            socket.emit(EVENT_KEYS.DELETE_SERVER, {success: true, projectName, serverName, hasDiffs });
    ...
    socket.on(EVENT_KEYS.DELETE_PARENT, async (arg) => {
        try {
            ...
            socket.emit(EVENT_KEYS.DELETE_PARENT, {success: true, projectName, hasDiffs, serverName, parentId });
    ...
    socket.on(EVENT_KEYS.UPDATE_PROJECT_SETTINGS, async (arg) => {
        try {
            ...
            const newSettings = {...projectSettings, ...settings, dataVersion: SUPPORTED_PROJECT_DATA_VERSION }
            await updateProjectSettings(projectName, newSettings);
            const hasDiffs = await hasUncommittedChanges(projectName)
    
            socket.emit(EVENT_KEYS.UPDATE_PROJECT_SETTINGS, {success: true, projectName, projectSettings: newSettings, hasDiffs});
