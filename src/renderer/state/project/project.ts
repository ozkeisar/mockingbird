...
import { PresetsFolder, PresetsFolderHash, ProjectServer, ProjectSettings, RouteParent, ServerSettings, ServersHash } from '../../../types';
...
export type StateFuncs = {
  ...
  updateRoute: (serverName: string, routes: RouteParent, filename: string) => void;
...
export const useProjectStore = create<State>((set) => ({
  ...
  updateRoute: (serverName: string, routeParent: RouteParent, filename: string)=>{
    set(
        produce<State>((state: State) => {
          state.serversHash[serverName].parentRoutesHash[routeParent.id] = routeParent;
