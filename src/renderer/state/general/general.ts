...
import { SelectedType, setSelectedPreset, setSelectedRouteType } from '../../../types/general';



export type StateFuncs = {
  resetGeneralState: () => void;
  setSelectedRoute: ({serverName, parentId, routeId}:setSelectedRouteType)=> void;
...
export const useGeneralStore = create<State>((set) => ({
    ...
    setSelectedRoute: ({serverName, parentId, routeId}: setSelectedRouteType)=>{
        set(
            produce<State>((state: State) => {
