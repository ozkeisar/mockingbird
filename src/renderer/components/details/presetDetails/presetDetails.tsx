...
function groupPresetRoutesByServerAndParent(
  routes: PresetRoute[],
): Record<string, Record<string, PresetRoute[]>> {
...
export function PresetDetails() {
  ...
  const [isDeletePresetDialogOpen, setIsDeletePresetDialogOpen] =
    useState(false);
  const [deleteRouteId, setDeleteRouteId] = useState<string | null>(null);
  const [presetRouteData, setPresetRouteData] = useState<PresetRoute | null>(
    null,
  );
  const [isPresetDialogOpen, setIsPresetDialogOpen] = useState(false);
  const [isApplyLoading, setIsApplyLoading] = useState(false);

  const data = groupPresetRoutesByServerAndParent(
    Object.values(preset?.routesHash || {}),
  );
  ...
  useEffect(() => {
    ...
    return () => {
      socket.off(EVENT_KEYS.APPLY_PRESET, onEvent);
    };
  }, [activeProjectName]);
  ...
  const renderRoute = (
    ...
    const routeTitle =
      routeExist &&
      `${routeItem.routePath}${routeItem.withParams ? routeParams : ''}`;
    ...
    return (
      <div className={styles.routeBlock} key={presetRoute.routeId}>
        <Card variant="outlined">
          <CardActionArea
            onClick={() => {
              reportElementClick(ELEMENTS.PRESET_DETAILS_ROUTE_ROW, {
                routeExist,
              });
              if (!routeExist) {
                return;
              ...
              setSelectedRoute({
                serverName: presetRoute.serverId,
                routeId: presetRoute.routeId,
                parentId: presetRoute.parentId,
              });
            }}
          >
            <div className={styles.routeDetailsCard}>
              {routeExist ? (
                <>
                  <div
                    className={styles.routeMethod}
                    style={{
                      backgroundColor: getRouteBGColor(routeItem.method),
                    }}
                  >
                    {routeItem.method}
                  </div>
                    ...
                    <div className={styles.routeTitle}>
                      ...
                      {!!routeItem?.withParams && (
                        <div className={styles.params}>
                          <div>
                            {`{${routeItem?.paramType}.${routeItem?.paramKey} = ${routeItem?.paramValue}}`}
                          </div>
                        </div>
                      )}
                    </div>
              ...
              <div className={styles.actionsWrapper}>
                {routeExist && responseExist ? (
                  <Tooltip title="selected response">
  ...
  const renderGraphqlRoute = (
    presetRoute: PresetRoute,
    server: ProjectServer,
    parent: RouteParent,
    routeItem: GraphQlRoute | undefined,
    response: GraphQlRouteResponse | undefined,
  ) => {
    const routeExist = !!routeItem;
    const responseExist = !!response;
              ...
              {routeExist ? (
                <>
                  <div
                    className={styles.routeMethod}
                    style={{
                      backgroundColor: getGraphqlRouteBGColor(routeItem.type),
                    }}
                  >
                    {routeItem.type}
                  </div>
                  <Tooltip title={routeItem?.name}>
                    <div className={styles.routeTitle}>{routeItem.name}</div>
                  </Tooltip>
                </>
              ) : (
                <div className={styles.roteDeletedTitle}>route deleted</div>
    ...
    <>
      <div className={styles.container}>
        <div className={styles.topPage}>
          ...
          <Tooltip
            title="some routes not exists"
            disableHoverListener={isAllRoutesExists}
          >
            <span>
              <LoadingButton
            ...
            return (
              ...
              >
                {!server ? (
                  ...
                  <div className={styles.serverTitleWrapper}>
                    <CloudQueueIcon />
                    <Typography className={styles.serverTitle} variant="h6">
                      {server.name}
                    </Typography>
                  </div>
                )}
                    ...
                    return (
                      <div className={styles.parentSection}>
                        <Link
                          ...
                          onClick={() => {
                            reportButtonClick(
                              BUTTONS.PRESET_DETAILS_PARENT_LINK,
                            );
                            setSelectedRoute({
                        ...
                        >
                          {isGraphql
                            ? parent.name
                            : parent?.path || 'deleted parent'}
                        </Link>

                        {_item.map((presetRoute) => {
                          if (isGraphql) {
                            const route =
                              parent?.graphQlRouteHash?.[presetRoute.routeId];
                            const response =
                              route?.responsesHash?.[presetRoute.responseId];

                            return renderGraphqlRoute(
                              presetRoute,
                              server,
                              parent,
                              route,
                          ...
                          const route =
                            parent?.routesHash?.[presetRoute.routeId];
                          const response =
                            route?.responsesHash?.[presetRoute.responseId];
      ...
      {isPresetRouteDialogOpen && !!presetFolder && !!preset && (
        <PresetRouteDialog
          open
          onClose={() => {
      ...
      {isPresetDialogOpen && !!presetFolder?.id && (
        <PresetDialog
          open
          data={preset || null}
      ...
      {isDeleteRouteDialogOpen && presetFolder && preset && deleteRouteId && (
        <DeletePresetRouteDialog
          open
          onClose={() => {
      ...
      {isDeletePresetDialogOpen && presetFolder && preset && (
        <DeletePresetDialog
          open
          onClose={() => {
