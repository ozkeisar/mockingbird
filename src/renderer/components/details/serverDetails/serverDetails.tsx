...
import { Divider, FormControlLabel, IconButton, Switch, TextField, Tooltip, Typography } from '@mui/material';
...
export const ServerDetails = ()=>{
    ...
    const { updateServerSettings, activeProjectName, setHasDiffs } = useProjectStore()
    const [_proxyBaseUrl, setProxyBaseUrl] = useState(server?.settings.proxyBaseUrl || '');
    const [_forceProxy, setForceProxy] = useState(server?.settings.forceProxy || false);
    const [_duplicateCookies, setDuplicateCookies] = useState(server?.settings.duplicateCookies || false);
    const [_reWriteCookieDomain, setReWriteCookieDomain] = useState(server?.settings.reWriteCookieDomain || false);
    const [_simplifyCookies, setSimplifyCookies] = useState(server?.settings.simplifyCookies || false);
    ...
    useEffect(()=>{
        const onEvent = (arg: any) => {
            setIsLoading(false);
            const {success, serverSettings, projectName, serverName, hasDiffs} = arg;
            setHasDiffs(hasDiffs)

            if(success && projectName === activeProjectName && serverName === server?.name ){
    ...
    const isDataModified = server?.settings.proxyBaseUrl !== _proxyBaseUrl 
    ...
    return ( 
       <div className={styles.container}>
           ...
           {isDeleteServerDialogOpen && !!server && <DeleteServerDialog 
