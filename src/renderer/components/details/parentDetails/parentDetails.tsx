...
export function ParentDetails() {
  const { parent, server } = useSelectedRoute();
  ...
  const [isDeleteParentDialogOpen, setIsDeleteParentDialogOpen] =
    useState(false);
  ...
  const renderGraphqlRoutes = () => {
    return Object.values(parent?.graphQlRouteHash || {})?.map(
      (routeItem, index) => {
        return (
          <div className="route-block" key={routeItem.id}>
            <Card variant="outlined">
              <CardActionArea
                onClick={() => {
                  reportElementClick(ELEMENTS.PARENT_DETAILS_ROUTE_ROW, {
                    index,
                  });
              ...
              >
                <div className="route-details-card">
                  ...
                  <div className="route-path">{routeItem.name}</div>
          ...
          <Card variant="outlined">
            ...
            >
              <div className="route-details-card">
                <div
                  className="route-method"
                  style={{ backgroundColor: getRouteBGColor(routeItem.method) }}
                >
                  {routeItem.method}
                </div>
                <div className="route-path">{routeItem.routePath}</div>
  ...
  const schemaPath = createPath(
    `${parent?.graphqlQueriesType}.${parent?.schemaPath}` || '',
    ...
    <>
      <div className="routes-container">
        <div className="route-header">
          <div>
            {isGraphql && (
              <Typography variant="caption">{parent?.path}</Typography>
            )}
            <Typography variant="h5" gutterBottom>
              {isGraphql ? parent.name : parent?.path}
            </Typography>
          </div>
          <div>
        ...
        {isGraphql && (parent?.schemaPath?.length || 0) > 0 && (
          <CopyBlock
            text={schemaPath}
            language="graphql"
            showLineNumbers
        ...
        <Button
          ...
          variant="contained"
        >
          Add {isGraphql ? parent.graphqlQueriesType : 'Route'}
        </Button>
      </div>
      {isDeleteParentDialogOpen && !!parent && !!server && (
        <DeleteParentDialog
      ...
      {isRouteDialogOpen && !!parent && !!server && !isGraphql && (
        <RouteDialog
