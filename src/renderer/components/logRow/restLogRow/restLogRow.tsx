...
import { Method, Route, RouteParent, RouteResponse, ServerLog } from "../../../../types";
import './restLogRow.css';
import JsonView from '@uiw/react-json-view';
import { vscodeTheme } from '@uiw/react-json-view/vscode';
import { Button, IconButton, Tooltip } from "@mui/material";
import { formatDate, getKeyValuePairReq, removeLastPartOfUri, removeQueryParams, reportButtonClick, reportElementClick } from "../../../utils";
...
function compareUrls(url1: string, url2: string) {
    ...
    for (let i = 0; i < url1Segments.length; i++) {
        ...
        if (segment1 !== segment2 && !isParameter(segment1) && !isParameter(segment2)) {
            return false;
        }
    }
...
type props = {
    ...
    onAddParentClick: (data:{data: Partial<RouteParent>, serverName?: string})=>void;
    onAddRouteClick: ({data , matchedParent}:{data: Route, matchedParent: RouteParent,  serverName: string})=>void;
    onAddResponseClick: ({}:{matchedParent: RouteParent, matchedRoute: Route, data: RouteResponse,  serverName: string})=>void;
}

export const RestLogRow = ({ openRows, data, index, setSize, windowWidth,onRowClick, onAddParentClick,onAddRouteClick,onAddResponseClick}: props) => {
    ...
    const matchedParents = useMemo(()=>Object.values(routesParents)?.filter((parent)=> requestUrl.includes(parent.path)),[routesParents]);

    const {matchedRoute, matchedParent} = useMemo(()=> matchedParents?.reduce((acc,parent)=>{
        const requestRoute = removeQueryParams(requestUrl).replace(parent.path,'');
       
        if(!!acc.matchedRoute){
            return acc
        }

        const route = Object.values(parent.routesHash || {}).find((route)=>{
            return compareUrls(route.routePath, requestRoute)
                && route.method === requestMethod
                && isParamMatching(route,logData)
            })
    ...
    },{matchedRoute: null, matchedParent: null} as {matchedRoute:Route | null, matchedParent: RouteParent| null}),[matchedParents]);
    ...
    const handleAddResponse = (e: React.MouseEvent<HTMLElement>)=>{
        ...
        const matchedParent = sortedMatchedParent[0];
        const requestRoute = removeQueryParams(requestUrl).replace(matchedParent.path,'');
        const matchedRoute = Object.values(matchedParent.routesHash || {}).find((routeItem)=>{
            return compareUrls(routeItem.routePath, requestRoute)
                && routeItem.method === requestMethod
                && isParamMatching(routeItem,logData)
        })
        if(!matchedRoute || !server){
            return;
        }
        onAddResponseClick({
    ...
    const handleAddRoute = (e: React.MouseEvent<HTMLElement>)=>{
        ...
        const {key, value, type} = getKeyValuePairReq(body, query, params)
        ...
        onAddRouteClick({
            ...
            data:{
                ...
                paramType: type,
                paramValue: value,
                routePath: removeQueryParams(requestUrl).replace(matchedParent.path,''),
        ...
        <div className={'collapse-header'} onClick={()=>{
            ...
            <div className="type-e-url">
                ...
                <Tooltip title={logData.request.url}>
                    <div>
                    <div style={{fontSize: '10px', lineHeight: '10px', marginBottom: '-2px'}}>{logData.metadata.serverName}</div>
                    <div className="row-url">{logData.request.url}</div>
                    </div>
                </Tooltip>
               {!!matchedRoute && matchedParent && <Tooltip title="open route">
                    <IconButton
                        edge="start"
                        color="inherit"
                        onClick={(e)=>{
                            e.stopPropagation();
                            setSelectedRoute({serverName, parentId: matchedParent.id, routeId: matchedRoute.id})
                        }}
                        aria-label="close"
                        size='small'
            ...
            <div className="time-e-add">
               {
                ...
                {!routeExist && <Button variant='text' color="inherit" onClick={handleAddRoute} disabled={!matchedParents?.length}>
                    add route
                </Button>}
                <Button variant='text' color="inherit" onClick={handleAddResponse} disabled={!routeExist}>
                    Add Response
                </Button>
