...
import { GraphQlRoute, GraphQlRouteResponse, GraphQlRouteType, Method, RouteParent, ServerLog, ServerLogType } from "../../../../types";
import styles from './graphqlLogRow.module.css';
import JsonView from '@uiw/react-json-view';
import { vscodeTheme } from '@uiw/react-json-view/vscode';
import { Button, IconButton, Menu, MenuItem, Tooltip } from "@mui/material";
import { formatDate, getGraphqlRouteBGColor, ParsedQuery, removeLastPartOfUri, reportButtonClick, reportElementClick } from "../../../utils";
import { useProjectStore } from "../../../state/project";
import { BUTTONS, ELEMENTS } from "../../../../consts/analytics";
import { parse } from 'graphql'
import { constructObject, determineQueryType, extractVariables, findAllParents, findParentTest, QueryData } from "../../../utils/graphql";
...
type props = {
    ...
    onAddParentClick: ({serverName, data }:{data: Partial<RouteParent>, serverName?: string})=>void;
    onAddQueryClick: ({data , matchedParent}:{data: Partial<GraphQlRoute>, matchedParent: RouteParent,  serverName: string})=>void;
    onAddQueryResponseClick: ({data, matchedParent, matchedRoute, serverName}: {serverName: string, matchedParent: RouteParent, matchedRoute: GraphQlRoute, data: Partial<GraphQlRouteResponse>})=>void
}


export const GraphqlLogRow = ({ openRows, data, index, setSize, windowWidth,onRowClick, onAddParentClick, onAddQueryClick, onAddQueryResponseClick}: props) => {
    ...
    const [menuType, setMenuType] = useState<'parent' | 'route' | 'response' | null>(null)
    ...
    const queryObj = constructObject(parsedQueryObj, extractVariables(parsedQueryObj))
    const queriesData = findParentTest(queryObj,'','', requestUrl,queryType,0,server)
    const {parents, responses, routes} = findAllParents(queryObj,'','', requestUrl,queryType,0,server);
    ...
    const handleClose = (event: React.MouseEvent<HTMLLIElement | HTMLButtonElement>) => {
        event.stopPropagation();
        setAnchorEl(null);
    };
    ...
    const handleAddResponseClick = (event: React.MouseEvent<HTMLButtonElement>) => {
        setMenuType('response')
        handleClick(event)
    };


    const handleAddResponse = (event: React.MouseEvent<HTMLLIElement | HTMLButtonElement>, item: QueryData | undefined)=>{
        ...
        const resPath = item.schemaPath?.length > 0 ? item.schemaPath+'.'+item.key : item.key
    ...
    const handleAddParent = (event: React.MouseEvent<HTMLLIElement | HTMLButtonElement>, item: ParsedQuery)=>{
        reportButtonClick(BUTTONS.CONSOLE_LOG_ROW_ADD_PARENT)
        handleClose(event)
        
        const path = removeLastPartOfUri(requestUrl)
        const name = item.schemaPath?.split('.')?.pop?.()?.split('(')[0] || item.key
    ...
    const handleAddRoute = (event: React.MouseEvent<HTMLLIElement | HTMLButtonElement>, item: QueryData)=>{
      ...
      >
                        ...
                        {queriesData.map((queryData)=>{
                            return (
                                <>
                                    ...
                                    {!!queryData.parent && !!queryData.route && <Tooltip title="open route">
                                        <IconButton
                                            edge="start"
                                            color="inherit"
                                            onClick={(e)=>{
                                                ...
                                                if(queryData.parent?.id && queryData.route?.id){
                                                    setSelectedRoute({serverName, parentId: queryData.parent.id, routeId: queryData.route.id})
                                                }
                                            }}
                                            aria-label="close"
                                            size='small'
            ...
            <div className="time-e-add">
               {
                ...
                <Button variant='text' color="inherit" onClick={handleAddRouteClick} disabled={!routes.length}>
                    add {queryType}
                </Button>}
                <Button variant='text' color="inherit" onClick={handleAddResponseClick} disabled={queriesData.every((item)=>!item.parent || !item.route)}>
                    Add Response
                </Button>
        ...
        >
            {routes.map((item)=>{
                ...
                }}>{item.schemaPath ? item.schemaPath +'.':''}{item.key}</MenuItem>
            })}
        </Menu>
        <Menu
            id="basic-menu"
