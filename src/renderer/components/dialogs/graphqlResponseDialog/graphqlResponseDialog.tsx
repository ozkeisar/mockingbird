...
import { GraphQlRoute, GraphQlRouteResponse, ProjectServer, ResponseType, RouteParent } from "../../../../types";
import Alert from '@mui/material/Alert';
import { v4 as uuid } from 'uuid';
import AutoFixHighIcon from '@mui/icons-material/AutoFixHigh';
import { prepareObjectToSchema, getGraphqlRouteBGColor, reportButtonClick, combineNestedObjects } from '../../../utils';
...
export const GraphqlResponseDialog = ({onClose, open, data, parent, route, server }:Props)=>{
    ...
    const [schemaTypeName, setSchemaTypeName] = useState<string>(data?.schemaTypeName || '');
    const execRef = useRef( data?.exec || '(arg, context, info) => {\n\treturn {\n\t\t\n\t}\n}')
    ...
    const existingNames = Object.values(route.responsesHash || {}).map((item)=>item.name)
    ...
    const generateTypes = (response: string)=>{
        ...
        const rootName =  nameArr.map((word): string => `${word[0].toUpperCase()}${word.slice(1)}`).join('_');
        ...
        const schemaArr = getGraphqlSchemaFromJsonSchema({rootName, schema: jsonSchemaObject})

        return {types: schemaArr.typeDefinitions.join('\n'), typeName: schemaArr.typeName }
    }
    ...
    const renderInputSide = ()=>(
        <div style={{width: '28%', height:'100%'}}>
            ...
            <TextField
                ...
                onChange={(e)=>{setSchemaTypeName(e.target.value)}}
            />
            {!isSchemaTypeNameValid && !typesSyntaxError  && <Typography color={'error'} variant="body2">response type name doesn't exist in response type</Typography>}
            ...
            <ToggleButtonGroup
                ...
                >
                <ToggleButton value="func">
                    Function
                </ToggleButton>
                <ToggleButton value="obj">
                    JSON
                </ToggleButton>
    ...
    const renderResponseEditor = ()=>(
        <div style={{width: '35%', height:'100%', position:'relative'}}>
            ...
            {!!badSyntaxError && <div style={{position:'absolute', width:'100%', bottom:'0'}}><Alert severity="error">{badSyntaxError}</Alert></div>}
            
            {resType === 'obj' && !badSyntaxError && <Tooltip arrow placement="top" title="generate response type">
                <Fab 
                style={{
                    position:'absolute',
                    bottom: 10,
    ...
    const renderTypesEditor = ()=>(
        <div style={{width: '35%', height:'100%', position:'relative'}}>
            ...
            />
            {!!typesSyntaxError && <div style={{position:'absolute', width:'100%', bottom:'0'}}><Alert severity="error">{typesSyntaxError}</Alert></div>}
        </div>
    )
    ...
    return (
        <Dialog
            ...
            <DialogContent style={{width:'100%',height:'75vh'}}>
                <div style={{width:'100%',height:'100%', display:'flex', overflow: 'hidden', justifyContent:'space-between'}}>
                    {renderInputSide()}
            ...
            <DialogActions>
                ...
                <Button disabled={hasError || !!badSyntaxError || !!typesSyntaxError || !isSchemaTypeNameValid || name.length < 1} type="submit" onClick={handleSave}>save</Button>
            </DialogActions>
        </Dialog>
    )
  }
