...
import { ProjectServer, ResponseType, Route, RouteParent, RouteResponse } from "../../../../types";
...
export const ResponseDialog = ({onClose, open, data, parent, route, server }:Props)=>{
    const [resType, setResType] = useState<ResponseType>( data?.type || 'obj' );
    const [name, setName] = useState<string>(data?.name || '');
    const [description, setDescription] = useState<string>(data?.description || '');
    const execRef = useRef( data?.exec || '(req, res) => {\n\t\n\tres.status(200).send({ isFunc: true })\n}')
    const [exec, setExec] = useState(execRef.current);
    const resRef = useRef(JSON.stringify(data?.res || {code: 200, data: {}, headers: {}}, null, 2))
    ...
    const renderTitleName = ()=>(
        <div className="route-header-title">
            {!!route?.method && <div className="route-method" style={{backgroundColor: getRouteBGColor(route.method)}}>
            {route.method}
            </div>}
            ...
            &&<div className="route-params">
                <div>{'{'+route?.paramType + '.' +route?.paramKey + ' = ' +route?.paramValue+'}'}</div>
            </div>
            } 
        </div>
    )
    ...
    return (
        <Dialog
            ...
            <DialogActions>
                ...
                <Button disabled={hasError || !!badSyntaxError} type="submit" onClick={handleSave}>save</Button>
            </DialogActions>
        </Dialog>
    )
  }
