import styles from './graphqlRouteDialog.module.css'
...
import { findMatchedGraphqlParent, getNonNativeTypesFromSignature } from '../../../utils/graphql';
...
export const GraphqlRouteDialog = ({onClose, open, data, parent, server }:Props)=>{
    const [description, setDescription] = useState<string>(data?.description || '');
    const [name, setName] = useState<string>(data?.name || '');
    const [type, setType] = useState<GraphQlRouteType | null>(data?.type || parent.graphqlQueriesType);
    const [selectedParent, setSelectedParent] = useState<RouteParent>(parent || Object.values(server.parentRoutesHash)[0]); 

    const isEdit = !!data?.id;

   
    const schemaPath = (parent.schemaPath?.length || 0) > 0 ? parent.schemaPath +'.'+ name : name
    const matchedParent = findMatchedGraphqlParent(schemaPath,parent.path,parent.graphqlQueriesType,server);

    const matchedQuery = Object.values(parent.graphQlRouteHash || {}).find((item)=>item.name === name)
       ...
       <Dialog
        ...
        <DialogContent style={{minWidth:'700px'}}>
            <div style={{display:'flex', alignItems:'center' }}>
                <FormControl disabled variant='filled' style={{width: '150px', marginBottom: '10px', marginRight: '13px'}}>
                    ...
                    >
                        {Object.values(server.parentRoutesHash).map((_parent)=>{
                            return (
                                <MenuItem value={_parent.id}>{_parent.name}</MenuItem>
                            )
                        })}
                    </Select>
                </FormControl>
                <FormControl variant='filled' disabled style={{width: '150px', marginRight: '10px', marginBottom: '10px'}}>
            ...
            { (!!matchedParent && name.length > 0)&& <Typography variant="subtitle2" gutterBottom style={{color:'red'}}>
                Schema path already exist on {matchedParent.name}
            </Typography>}
            {!!customTypeError && <Typography variant="subtitle2" gutterBottom style={{color:'yellow'}}>
