...
export function BranchDialog({
  ...
  const [_branches, setBranches] = useState<{ id: string; label: string }[]>(
    branches.map((label) => ({ id: label, label })),
  );
  ...
  useEffect(() => {
    const onEvent = (arg: any) => {
      ...
      if (success && projectName === activeProjectName) {
        setBranches(
          __branches.map((name: string) => ({ id: name, label: name })),
        );
        setHasDiffs(hasDiff);
  ...
  }, [activeProjectName, onClose, setHasDiffs]);
