...
import { CloneDialog, ServersIpDialog } from '../dialogs';
...
export function TopBar() {
  const { activeProjectName, hasDiffs, setHasDiffs, isGitInit } =
    useProjectStore();
  const {
    isServerUp,
    isServerLoading,
  ...
  const handleChangeProject = (event: SelectChangeEvent) => {
    ...
    emitSocketEvent(EVENT_KEYS.CHANGE_PROJECT, {
  ...
  return (
    <>
      <AppBar position="static" className="header">
        <Toolbar>
          <div className="title-n-pull">
              ...
              >
                ...
                >
                  {projectsNameList.map((projectName) => {
                    return (
                      <MenuItem id={projectName} value={projectName}>
                        {projectName}
                      </MenuItem>
                    );
                  })}
            ...
            {isGitInit && !!activeProjectName && !isServerUp && (
              <LoadingButton
            ...
            {isGitInit && !!activeProjectName && hasDiffs && !isServerUp && (
              <LoadingButton
                loadingPosition="start"
            ...
            {!isGitInit && !isServerUp && !!activeProjectName && (
              <LoadingButton
                loadingPosition="start"
      ...
      {isServersIpDialogOpen && (
        <ServersIpDialog
          open={isServersIpDialogOpen}
          onClose={() => {
            setIsServersIpDialogOpen(false);
          }}
        />
      )}
      {isAddRemoteDialogOpen && (
        <AddRemoteDialog open onClose={() => setIsAddRemoteDialogOpen(false)} />
      )}
      <Dialog
        open={openPullDialog}
      ...
      >
        <DialogTitle id="alert-dialog-title">Pull data from remote</DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            This will stash your local changes and get you the remote data. are
