...
import { ActivationDialog, IpChangedDialog, ServerDownDialog, SettingsDialog } from "../dialogs";
...
export const GlobalEvents = ()=>{
    ...
    const [openInvalidProjectDataDialog, setOpenInvalidProjectDataDialog] = useState(false);
    const [unsupportedProjectName, setUnsupportedProjectName] = useState('')
    const [serverDisabledUntil, setServerDisabledUntil] = useState<Date | null>(null);
    ...
    },[isServerUp, setServers, setActiveProjectName, setProjectSettings, setLoadingData]);


  useEffect(()=>{
    const onEvent = (arg: any) => {
      const {projectDataIsUnsupported, newProjectName, newProjectsNameList } = arg;
    ...
    useEffect(()=>{
      const onEvent = (arg: any) => {
        const {success, content, filename, serverName, projectName, hasDiffs } = arg as any;
        setHasDiffs(hasDiffs)
    ...
    },[socket, isServerUp, updateRoute, activeProjectName]);
    ...
    useEffect(()=>{
      ...
      socket.on(EVENT_KEYS.IP_CHANGED,onEvent);
  
      return ()=>{
        socket.off(EVENT_KEYS.IP_CHANGED, onEvent)
      }
    },[setOpenIpDialog, isServerUp]);

    useEffect(()=>{
      const onEvent = (arg: any) => {
        const {success, host, serverDisabledUntil, projectName } = arg;
    ...
    return (
        <>
          <IpChangedDialog open={openIpDialog} onClose={()=>{setOpenIpDialog(false)}}/>
          { openActivationDialog && <ActivationDialog open={openActivationDialog} 
            onClose={()=>{
              setOpenActivationDialog(false)

            }} />}
          {openServerDownDialog && !!serverDisabledUntil && <ServerDownDialog 
