...
import { IconButton, Link, ToggleButton, ToggleButtonGroup, Typography } from "@mui/material";
...
export const CloneProject = ({onCloneSuccess, onCancel}: {onCancel?: ()=>void; onCloneSuccess?: ()=> void}) => {
  ...
  const hasError = projectNameError || !projectName.length || (!directoryPath?.length && cloneType === 'OPEN')
    ...
    useEffect(()=>{
        const removeListener = ElectronEvents.on(EVENT_KEYS.SELECT_DIRECTORY, (arg) => {
            const { success, directoryPath } = arg as any;
            if(success){
    ...
    const handleCloneClick = ()=>{
        ...
        emitSocketEvent(EVENT_KEYS.CREATE_PROJECT, {sshUrl, httpsUrl, cloneType, username, password, projectName, directoryPath});    
    }
        ...
        <div className="empty-state-container">
           {!!onCancel &&  <IconButton
            ...
            <div className="clone-container">
                <Typography  variant="h6" gutterBottom>{getActionName()} your Mockingbird project</Typography>
                <ToggleButtonGroup
                    value={cloneType}
                    exclusive
                ...
                {
                cloneType ==='HTTPS' && <div className={styles.inputsContainer}>
                    <TextField 
                    ...
                    <TextField 
                        ...
                        fullWidth
                    />
                    <Typography style={{marginBottom: '15px', marginTop:'5px'}} variant="caption" gutterBottom>{"Example repo "}
                        <Link>
                        { httpsExample }
                        </Link>
                    </Typography>
                ...
                { cloneType ==='LOCAL' && <div className={styles.inputsContainer}>
                        <TextField 
                            style={{marginBottom: '15px'}}
                            value={projectName}
                            onChange={(e)=>{
                ...
                { cloneType ==='OPEN' && <div className={styles.inputsContainer}>
                        <TextField 
                            style={{marginBottom: '15px'}}
                            value={projectName}
                            onChange={(e)=>{
                        ...
                        >
                            {directoryPath?directoryPath:'select project directory'}
                        </Link>
                    </div>
                }   
