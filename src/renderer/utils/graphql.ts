import { GraphQlRoute, GraphQlRouteType, ProjectServer, RouteParent } from "../../types";
import { removeParameters } from "../../utils/utils";
import { isSomeChildrenLeafs, mergeObjects, ParsedQuery, removeSquareBrackets } from "./general";
...
export const findMatchedGraphqlParent = (schemaPath: string, path: string, type: GraphQlRouteType | null, server: ProjectServer)=>{
    return Object.values(server.parentRoutesHash).find((parent)=>{

        return parent.type === 'GraphQl' 
        && parent.path === path 
...
export const findMatchedGraphqlRoute = (schemaPath: string, path: string, type: GraphQlRouteType, server: ProjectServer)=>{
    const route = Object.values(server.parentRoutesHash).reduce((acc, parent)=>{
        ...
        if (parent.type === 'GraphQl' 
            ...
            && queries.length > 0){
            const route = queries.find((query)=>{
              return removeParameters(parent.schemaPath+'.'+query.name) === removeParameters(schemaPath)})
            return acc || route || null;
        }
...
const findMatchQueryOnParent = (queryName: string, parent: RouteParent)=>{
    const queries = Object.values(parent.graphQlRouteHash || {});
    return queries.find((query)=>query.name.split('(')[0] === queryName.split('(')[0])
}

export const findParentTest = (obj: TreeObject|null, key:string, schemaPath: string, path: string, type: GraphQlRouteType, level: number, server: ProjectServer)=>{
    if(!obj){
        return [];
    }
    const isWrappers =[0,1].includes(level) 

    const parent = isWrappers ? null : findMatchedGraphqlParent(schemaPath, path, type, server)
    ...
    if(!isSomeChildrenLeafs(obj)){
        ...
        const queriesDataArr:QueryData[][] = Object.keys(obj)
            .map((_key)=>findParentTest(obj[_key],_key, isWrappers ? schemaPath : _schemaPath+key ,path, type, ++level,server));
        
        const queriesData = queriesDataArr.flat()

        if(queriesData.some(item=>!!item.route) || (queriesData.some(item=>!!item.parent) && !queryData.route)){
            return queriesData
        }
...
export const findAllParents = (obj: TreeObject|null, key:string, schemaPath: string, path: string, type: GraphQlRouteType, level: number, server: ProjectServer):Res=>{
  if(!obj){
  ...
  if(!!parent){
    const routeSchemaPath = (parent.schemaPath?.length || 0) > 0 ? parent.schemaPath +'.'+ key : key
    matchedParent = findMatchedGraphqlParent(routeSchemaPath, parent.path, parent.graphqlQueriesType, server);
  }
  ...
  if(!isSomeChildrenLeafs(obj)){
      ...
      const res = Object.keys(obj)
          .map((_key)=>findAllParents(obj[_key],_key, isWrappers ? schemaPath : _schemaPath+key ,path, type, ++level,server));
      ...
      const responses = res.map(resObj => resObj.responses).flat();
       ...
       return {
        parents: isWrappers ? parents :eliminateDuplicates([...parents, ...currentRes.parents]),
        routes: isWrappers ? routes : [...routes, ...currentRes.routes],
        responses: isWrappers ? responses : [...responses, ...currentRes.responses]
       }
  }
...
export const constructObject = (parsedQuery: any, variables: VariableInfo[]): {[key:string]: any} | null => {
    if (parsedQuery.kind === 'Document') {
      // Extracting the first definition (assuming only one query/mutation is present)
      const objects = parsedQuery.definitions.map((definition:any)=>constructObject(definition, variables));
  
      return mergeObjects(objects);
    } else if (parsedQuery.kind === 'OperationDefinition') {
      // Traversing the selection set
      return { [parsedQuery.name?.value]: constructObject(parsedQuery.selectionSet, variables)};
    } else if ( parsedQuery.kind === 'SelectionSet') {
      const objects = parsedQuery.selections.map((selection:any)=>constructObject(selection, variables))
      return mergeObjects(objects);
    } else if (parsedQuery.kind === 'Field') {
      ...
      const fieldName = parsedQuery.alias ? parsedQuery.alias.value : parsedQuery.name.value;
      return {
        [fieldName + params]: parsedQuery.selectionSet ? constructObject(parsedQuery.selectionSet, variables) : null
      };
    }
    return null
  }
...
export function validateGraphQLType(typeName: string, schemaString: string): boolean {
  // Regular expression to match type definitions
  const typeRegex = new RegExp(`type\\s+${removeSquareBrackets(typeName)}\\s*{([^}]+)}`, 'i');
...
export function buildQueryFromSchema(schema: string, initRootType: string = '', schemaPath: string, name: string): string {
  // Parse the schema
  ...
  let types: { [key: string]: {name: string, type: string}[] } = {};
  ...
  while ((match = typeRegex.exec(schema)) !== null) {
    ...
    while ((fieldMatch = fieldRegex.exec(typeFields)) !== null) {
      fields.push({name: fieldMatch[1], type: removeSquareBrackets(fieldMatch[2])});
    }
    
    types[typeName] = fields;
  }
  // Function to recursively build query
  function buildQueryForType(parent: {type: string, name: string}, indent: string = ''): string {
    if (!types[parent.type]) return '';
    
    let query = `${indent}${parent.name} {\n`;
    for (let field of types[parent.type]) {
      if (types[field.type]) {
  ...
  // Build the query
  let query: string;
  let indent = '  '.repeat(schemaPathLength + 1)
