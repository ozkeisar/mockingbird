...
export const isParentExist = (
  server: ProjectServer,
  _parentsProperties: ParentProperties,
  parent: RouteParent,
) => {
  const { filenames, paths, graphQlNames, restPaths } = _parentsProperties;

  const filenameAlreadyExist = filenames?.includes(
    parent.filename.toLowerCase(),
  );

  const nameAlreadyExist = graphQlNames?.includes(
    (parent.name || '').toLowerCase(),
  );

  const pathAlreadyExist = paths?.includes(parent.path.toLowerCase());

  const restPathAlreadyExist = restPaths?.includes(parent.path.toLowerCase());
