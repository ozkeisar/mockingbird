...
import { getCurrentIPAddresses, getProjectServersNameList, readProjectSettings, readServerData, readServerSettings } from '../backend/utils';
import axios from 'axios';
import { buildProxyResLog, buildRequestOptions, getLog, getSelectedRoute, handleGraphQlRoutes, handleResponse, sendLog, updateLog, updateResponseHeaders } from './utils';
import { Method, Route, RouteParent } from '../types';
import { hashmapToList } from '../backend/utils/utils';
...
function loggerMiddleware(projectName:string, req: Request, res: Response, next: NextFunction) {
    ...
    res.send = function(data: any) {
		try {
			if (!logged) {
				...
				sendLog(projectName, req, response, {
...
export const startProjectServers = async (projectName: string)=>{
	...
	servers = await Promise.all(projectServersNameList.map(async (serverName)=>{
...
export const startServer = async (projectName: string, serverName: string, host: string)=>{
	...
	const {restParents, graphQlParents} = Object.values(routeParentsHash).reduce((acc, item)=>{
		...
		return acc
	},{restParents: [], graphQlParents: []} as {restParents: RouteParent[] , graphQlParents: RouteParent[]})
	...
	restParents.forEach((routeParent)=>{
		...
		const switchRoutes = routes
			.filter((route)=>!!route?.paramType && !!route?.paramKey?.length && !!route?.paramValue?.length);
		const crudRoutes = routes
			.filter((route)=>!(!!route?.paramType && !!route?.paramKey?.length && !!route?.paramValue?.length));
	
		const routeObj = switchRoutes.reduce((acc,route)=>{
		...
		Object.keys(routeObj).forEach((key)=>{
			...
			router[method](routePath, async (req: Request, res: Response, next: NextFunction) => {
				const selectedRoute = getSelectedRoute(req, routeList);
				if(!!selectedRoute){
					handleResponse({host, serverName, route: selectedRoute, req, res, next, serverSettings});
				}
				next();
			});
		});
		...
		crudRoutes.forEach((route)=>{
			...
			router[method](routePath, async (req: Request, res: Response, next:NextFunction ) => {
				handleResponse({host, serverName, route, req, res, next, serverSettings});
			});
		});
	...
	app.use(async (req, res) => {
		...
		try {
			...
			res = updateResponseHeaders({res, headers: proxyRes.headers, host, proxyUrl: serverSettings?.proxyBaseUrl || '', serverSettings});
			...
			updateLog((req as any).id, {logType: 'error', proxyResponse:proxyResLog});
	...
	const server = app.listen(serverSettings.port as number, host, () =>{
		console.log(`--------app listening on http://${host}:${serverSettings.port}`)
	});
